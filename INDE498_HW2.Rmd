...
library(ggplot2)
newData <- AD.pred[order(AD.pred$PTGENDER),]
p <- ggplot(newData, aes(x = PTGENDER, y = AGE_bin))
# predicted curve and point-wise 95% CI
p <- p + geom_ribbon(aes(x = PTGENDER, ymin = fit.lower, ymax = fit.upper), alpha = 0.2)
p <- p + geom_line(aes(x = PTGENDER, y = fitted), colour="red")
# fitted values
...
# Visualizing the predication 
library(ggplot2)
newData <- AD.pred[order(AD.pred$PTEDUCAT),]
p <- ggplot(newData, aes(x = PTEDUCAT, y = AGE_bin))
# predicted curve and point-wise 95% CI
p <- p + geom_ribbon(aes(x = PTEDUCAT, ymin = fit.lower, ymax = fit.upper), alpha = 0.2)
p <- p + geom_line(aes(x = PTEDUCAT, y = fitted), colour="red")
# fitted values
p <- p + geom_point(aes(y = fitted), size=2, colour="red")
# observed values
p <- p + geom_point(size = 2)
